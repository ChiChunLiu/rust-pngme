...
impl Png {
    ...
    fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let other_chunk_type = ChunkType::from_str(chunk_type).unwrap();
        let index = self
            .chunks
            .iter()
            .position(|chunk| *chunk.chunk_type() == other_chunk_type);
        match index {
            Some(i) => {
                let chunk = self.chunks.remove(i);
                Ok(chunk)
            }
            None => Err(Box::from("No chunk of the specified type found")),
        }
    }
    fn header(&self) -> &[u8; 8] {
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        if header != Self::STANDARD_HEADER {
            return Err(Box::from("Header incompatible to png file"));
        }
        let mut chunks: Vec<Chunk> = Vec::new();
        while !bytes.clone().next().is_none() {
            let data_length_byte = bytes.by_ref().take(4).cloned().collect::<Vec<u8>>();
            let data_length = u32::from_be_bytes(data_length_byte.clone().try_into().unwrap());

            let rest_bytes = bytes
                .by_ref()
                .take((data_length + 8).try_into().unwrap())
                .cloned()
                .collect::<Vec<u8>>();

            let chunk_bytes = data_length_byte
                .iter()
                .cloned()
                .chain(rest_bytes.iter().cloned())
                .collect::<Vec<u8>>();
